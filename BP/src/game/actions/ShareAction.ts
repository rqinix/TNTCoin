...
export class ShareActionGui {
    ...
    private readonly _manager: EventActionManager<ShareAction>;
    ...
    constructor(player: Player, shareActionManager: EventActionManager<ShareAction>) {
        this._player = player;
        this._manager = shareActionManager;
        this._eventActionForm = new EventActionForm(player, shareActionManager);
    }

    public show(): void {
        this._eventActionForm.continueWithActions(this.createNewShareAction.bind(this), (actions) => {
            const shareActions = actions.get(this._eventKey) || [];
            ...
            form.button('Clear All Share Actions', () => this._eventActionForm.showClearAllActionsForm(this._manager.getAllActions()));
            form.show();
        });
    }

    private createNewShareAction(): void {
        new ActionForm(this._player, 'No Share Actions')
